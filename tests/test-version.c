...
void test_format_commit_only(void) {
    ZapVersion v = { .branch = Z_SV_NULL, .commit = Z_SV("29be27b"), .build = Z_BUILD_RELEASE };
    ZStringBuf sb;
    z_strbuf_init(&sb);
...
void test_format_branch_and_commit(void) {
    ZapVersion v = { .branch = Z_SV("main"), .commit = Z_SV("HEAD"), .build = Z_BUILD_RELEASE };
    ZStringBuf sb;
    z_strbuf_init(&sb);
    z_format_zap_version(v, &sb);
...
void test_format_commit_debug(void) {
    ZapVersion v = { .branch = Z_SV_NULL, .commit = Z_SV("c0200e9"), .build = Z_BUILD_DEBUG };
    ZStringBuf sb;
    z_strbuf_init(&sb);
    z_format_zap_version(v, &sb);
...
void test_format_branch_commit_debug(void) {
    ZapVersion v = { .branch = Z_SV("main"), .commit = Z_SV("HEAD"), .build = Z_BUILD_DEBUG };
    ZStringBuf sb;
    z_strbuf_init(&sb);
    z_format_zap_version(v, &sb);
