...
bool z_generate_batch_shim_for(ZPathView self, ZapToolchainElement tool, ZStringBuf* out) {
    ...
    if (!z_strbuf_appendf(out, "@echo off\r\n")) return false;
    if (!z_strbuf_appendf(out, ":: NOTE: This file is auto-generated by zapup and should not be modified directly\r\n")) return false;

    if (!z_strbuf_appendf(
            out,
            "for /f \"delims=\" %%%%i in ('" Z_SV_FMT " which " Z_SV_FMT "') do (\r\n",
            Z_SV_FARG(self),
            Z_SV_FARG(tool_string)
        )) return false;

    if (!z_strbuf_appendf(out, "  \"%%%%i\" %%*\r\n")) return false;
    if (!z_strbuf_appendf(out, "  exit /b %%ERRORLEVEL%%\r\n")) return false;
    if (!z_strbuf_appendf(out, ")\r\n")) return false;
